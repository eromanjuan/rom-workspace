...
export function openAddAppModal(wsId, user, onCreated) {
  ...
  body.append(el('div', { class: 'addapp-options' }, [
    ...
    optionCard('building-store', '#0891b2', 'ROMIO App Market', 'Browse apps shared by anyone and copy one into this workspace.',
...
function renderInstallFromFile(body, wsId, user, onCreated, close) {
  ...
  fileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      if (!parsed || !parsed.rom_app || !Array.isArray(parsed.fields)) throw new Error('Not a valid ROMIO app file.');
      def = parsed;
  ...
  body.append(el('div', { class: 'field-modal' }, [
    el('p', { class: 'muted' }, 'Upload a .rom.json file exported from any ROMIO app to recreate its structure (fields, reports, automations) here. Items are not included.'),
